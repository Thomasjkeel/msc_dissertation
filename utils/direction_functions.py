...
def mean_direction(linestring, plot_windrose=False):
    ...
    if type(linestring) == shapely.geometry.LineString:
        xys = np.array(linestring.xy).T # get all the x,y coordinates in an array of arrays
   ...
   # else:
    ...
    res_dir, res_mag = find_resultant(line_to_bearings, line_mag) # calculate the resultant for mean direction
